...
export default function HistorialPublico() {
  ...
  const publicThoughts = allThoughts.filter(t => !t.private);
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingTop: 24 }}>
        ...
        >
          <View style={styles.headerRow}>
            ...
            <MaterialCommunityIcons name="earth" size={20} color="#fff" style={{ marginRight: 8, marginLeft: 2 }} />
            <Text style={styles.headerTitle}>Historial Público</Text>
          </View>
        </LinearGradient>
        {publicThoughts.length === 0 ? (
          <Text style={{ color: colors.secondaryText, textAlign: 'center', marginTop: 24 }}>
            No tienes pensamientos públicos aún.
          </Text>
        ) : (
